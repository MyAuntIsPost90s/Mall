...
public class OrderGoodsServiceImpl implements OrderGoodsService {
    ...
    public EUIPageList<Ordergoods> list(Ordergoods ordergoods, int page, int rows) {
        PageList<Ordergoods> pageList = QueryPageListProxy.query(() -> ordergoodsMapper.getList(ordergoods), page, rows);
        return new EUIPageList<Ordergoods>(pageList.getTotal(), pageList.getRows());
    }

    @Override
    public EUIPageList<Ordergoods> list4count(String userid, String ordergoodsname, int page, int rows) {
        PageList<Ordergoods> pageList = QueryPageListProxy.query(() -> ordergoodsMapper.list4count(userid, ordergoodsname), page, rows);
